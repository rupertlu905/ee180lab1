...
radsort: 
    ...
    addi $sp, $sp, -32 #change offset when we have more stuff
    sw $ra, 16($sp)
    sw $a2, 20($sp)
    sw $a1, 24($sp)
    sw $a0, 28($sp)

    # Move a0 into s register
...
find_exp:
    # leaf procedure (no s registers here)
...
exp_while_loop:
    sltu $t6, $s8, $s1    # Check if radix <= largest
    beq $t6, $zero, exp_exit2
    
    divu $s1, $s1, $s8    # largest = largest / RADIX
    mflo $s1              # move quotient to largest
    
    mul $t5, $t5, $s8     # exp = exp * RADIX
